...
type EditInstallmentDueDateModalProps = {
  open: boolean;
  installment: PaymentInstallment | null;
  loading?: boolean;
  onClose: () => void;
  onSubmit: (payload: { dueDate: string; reason?: string }) => void;
};

export function EditInstallmentDueDateModal({
  open,
  installment,
  loading,
  onClose,
  onSubmit,
}: EditInstallmentDueDateModalProps) {
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl border border-slate-100 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div>
            <h3 className="text-lg font-black text-slate-900">Edit Due Date</h3>
            <p className="text-xs font-bold text-slate-400 mt-1">
              Installment #{installment.installmentNumber} · Current due date{" "}
        ...
        <div className="p-6 space-y-4">
          ...
          <label className="block space-y-2">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Why is this due date being changed?"
        ...
        <div className="px-6 py-5 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3">
          ...
          <button
            onClick={() => onSubmit({ dueDate, reason })}
